...
export default function AddOfferScreen() {
  ...
  function validateForm() {
    ...
    if (!imageBase64) {
      nextErrors.image = "Velg et tilbudsbilde.";
    }

    if (!title.trim()) {
      nextErrors.title = "Skriv inn en tittel.";
    }
    ...
    if (!oldPrice.trim()) {
      nextErrors.oldPrice = "Skriv inn ordinærpris.";
    } else if (Number.isNaN(parsedOldPrice) || parsedOldPrice <= 0) {
      nextErrors.oldPrice = "Prisen må være et gyldig tall større enn 0.";
    }

    if (!offerPrice.trim()) {
      nextErrors.offerPrice = "Skriv inn tilbudspris.";
    } else if (Number.isNaN(parsedOfferPrice) || parsedOfferPrice <= 0) {
      nextErrors.offerPrice = "Prisen må være et gyldig tall større enn 0.";
    } else if (
      !Number.isNaN(parsedOldPrice) &&
      parsedOldPrice > 0 &&
      parsedOfferPrice >= parsedOldPrice
    ) {
      nextErrors.offerPrice =
        "Tilbudsprisen må være lavere enn ordinærprisen.";
    }

    if (!duration.trim()) {
      nextErrors.duration = "Skriv hvor lenge tilbudet gjelder.";
    }
  ...
  async function saveOffer() {
    ...
    try {
      ...
      clearForm();
      setSubmitError("");
      setSuccessMessage("Tilbudet ble publisert og lagret.");
    } catch (error) {
      console.error("Feil ved lagring av tilbud:", error);

      setSuccessMessage("");
      setSubmitError(
        "Kunne ikke publisere tilbudet. Kontroller at du er logget inn som admin og prøv igjen."
      );

      Alert.alert(
        "Kunne ikke publisere",
        "Tilbudet ble ikke lagret. Se feilmeldingen i skjemaet."
      );
    } finally {
      setIsSaving(false);
    }
    ...
    setShowDatePicker(false);
        ...
        <View style={styles.priceRow}>
          <View style={styles.priceField}>
            ...
            />
            {errors.oldPrice ? (
              <Text style={styles.errorText}>{errors.oldPrice}</Text>
            ) : null}
          </View>
          ...
          <View style={styles.priceField}>
            ...
            <TextInput
              style={[styles.input, errors.offerPrice && styles.inputError]}
              value={offerPrice}
              onChangeText={(value) => {
                setOfferPrice(value);
            ...
            />
            {errors.offerPrice ? (
              <Text style={styles.errorText}>{errors.offerPrice}</Text>
            ) : null}
          </View>
        </View>
          ...
          <View style={styles.autoHideToggleRow}>
            ...
            <View style={styles.autoHideTextContainer}>
              ...
              <Text style={styles.autoHideDescription}>
                Tilbudet blir satt som skjult når du åpner adminpanelet
                etter valgt dato.
              </Text>
            </View>
          </View>
              ...
              <View style={styles.dateModalHeader}>
                <View style={styles.dateModalHeaderText}>
                  ...
                  <Text style={styles.dateModalSubtitle}>
                    Velg dagen tilbudet skal skjules.
                  </Text>
                </View>
