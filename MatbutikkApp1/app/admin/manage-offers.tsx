...
export default function ManageOffersScreen() {
  ...
  async function deleteOffer(offer: Offer) {
    ...
    try {
      ...
      if (Platform.OS === "web") {
        window.alert(`«${offer.title}» ble slettet permanent.`);
      } else {
        Alert.alert(
          "Tilbud slettet",
      ...
      const message =
        "Tilbudet ble ikke slettet. Kontroller admin-tilgangen og prøv igjen.";

      if (Platform.OS === "web") {
        window.alert(message);
      } else {
        Alert.alert("Kunne ikke slette", message);
      }
    } finally {
      setUpdatingOfferId(null);
    }
