...
export default function AdminScreen() {
  ...
  async function hideExpiredContent() {
    const now = new Date();

    async function processCollection(
      collectionName: "offers" | "products"
    ) {
      ...
      const updates = snapshot.docs
        .map((documentSnapshot) => {
          ...
          const hideAt =
            data.hideAt instanceof Timestamp
              ? data.hideAt.toDate()
              : null;

          const shouldHide =
            data.autoHideEnabled === true &&
            data.isActive === true &&
            hideAt !== null &&
            hideAt.getTime() <= now.getTime();
          ...
          return updateDoc(
            doc(db, collectionName, documentSnapshot.id),
            {
              isActive: false,
    ...
    try {
      await Promise.all([
        processCollection("offers"),
        processCollection("products"),
      ]);
    } catch (error) {
      console.error(
        "Kunne ikke skjule utløpt innhold automatisk:",
        error
      );
    }
  }
